...
from CoinStock import CoinStock
...
@app.get("/get-coinstocks")
def get_coinstocks():
    return coinstocks


@app.put("/update-coinstocks/{coinstocks_id}")
def update_propduct(coinstocks_id: int, updateCoinStock: UpdateCoinStock):
    if coinstocks_id not in coinstocks:
        return {"Error": "Product does not found"}

    if updateCoinStock.types != None:
        coinstocks[coinstocks_id]['types'] = updateCoinStock.types

    if updateCoinStock.value != None:
        coinstocks[coinstocks_id]['value'] = updateCoinStock.value

    if updateCoinStock.amount != None:
        coinstocks[coinstocks_id]['amount'] = updateCoinStock.amount
    return coinstocks
